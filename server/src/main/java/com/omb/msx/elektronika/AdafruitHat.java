...
public class AdafruitHat {
    private static final Logger LOGGER = LoggerFactory.getLogger(AdafruitHat.class);
     ...
     */
    public static final int LED0_ON_H = 0X07; //LED0 ON high byte
    /**
     * 12-bit low-byte for turn off time of LED0 PWM
     */
    public static final int LED0_OFF_L = 0X08; //LED0 OFF low byte
    /**
     * 12-bit high-byte for turn off time of LED0 PWM
     */
    public static final int LED0_OFF_H = 0X09; //LED0 OFF high byte
    public static final int LED1_ON_L = 0X0A; //etc.....
    ...
    private void checkDeviceAddr(int deviceAddr) {
        if (deviceAddr < 0X40 || deviceAddr > 0X7F) {
            LOGGER.error("*** Error *** Illegal AdafruitHat device address must be in rage 0X40 to 0X7F");
    ...
    private void checkBus(int i2cBus) {
        if (i2cBus != I2CBus.BUS_1) {
            LOGGER.error("*** Error *** - Illega I2C Bus address must be I2CBus.BUS_1 or I2CBus.BUS_2");
    ...
    private void setup() {
        try {
            ...
            if (mode1 < 0) {
                LOGGER.error("*** Error *** IC2 read returns negative value.");
            ...
            LOGGER.error("*** Error *** failed to commnicate with AdafruitHat device");
            stopAll();
            LOGGER.error("Hat error",e);
    ...
    public void setPwmFreq(double frequency) {
        ...
        if (frequency < 24.0 || frequency > 1526.0) {
            LOGGER.error("*** Error *** Frequency must be in range 24 HZ to 1526 HZ");
        ...
        if (preScale < 3 || preScale > 255) {
            LOGGER.error("*** Error *** PCA9685 prescale value must be in range 3 - 255");
        ...
        try {
            ...
            if (mode1 < 0) {
                LOGGER.debug("*** Error *** IC2 returns negative value.");
                throw new IOException(Integer.toString(mode1));
            }
            ...
            LOGGER.error("*** Error *** Can not I2C read from hatDevice");
            stopAll();
            LOGGER.error("Hat error",e);
        }
    ...
    public synchronized void write(int addr, byte value) {
        try {
            ...
            LOGGER.error("*** ERROR *** Can not perform I2C write to AdafruitHat Device");
            LOGGER.error("Hat error",e);
        }
    }
    ...
    public void sleep(long milliseconds) {
        try {
            ...
            LOGGER.error("*** ERROR *** Interrupted sleep");
            stopAll();
